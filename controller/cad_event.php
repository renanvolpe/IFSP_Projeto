<?php
session_start();

include_once '../conexao.php';

$dados = filter_input_array(INPUT_POST, FILTER_DEFAULT);
echo '<span> $dados </span>';
echo '<span> $dados </span>';
echo '<span> $dados </span>';
echo '<span> $dados </span>';
echo '<span> $dados </span>';
//Converter a data e hora do formato brasileiro para o formato do Banco de Dados
$data_start = str_replace('/', '-', $dados['start']);
$data_start_conv = date("Y-m-d H:i:s", strtotime($data_start));

$data_end = str_replace('/', '-', $dados['end']);
$data_end_conv = date("Y-m-d H:i:s", strtotime($data_end));

$data_paciente_conv=(int)$dados['paciente_idPaciente'];
$data_medico_conv=(int)$dados['medico_idMedico'];

$query_event = "INSERT INTO agenda (title, color, start, end, paciente_idPaciente, medico_idMedico) VALUES (:title, :color, :start, :end, :paciente_idPaciente, :medico_idMedico)";

//$verificacao = "SELECT start, end FROM agenda WHERE (start < $data_start_conv AND end > $data_start_conv) OR (start < $data_end_conv AND end > $data_end_conv)";
//$verificacao = $conn->query($verificacao);

//if($verificacao->num_rows == 0) {

$insert_event = $conn->prepare($query_event);
$insert_event->bindParam(':title', $dados['title']);
$insert_event->bindParam(':color', $dados['color']);
$insert_event->bindParam(':start', $data_start_conv);
$insert_event->bindParam(':end', $data_end_conv);
$insert_event->bindParam(':paciente_idPaciente', $data_paciente_conv);
$insert_event->bindParam(':medico_idMedico',$data_medico_conv );


//}

if ($insert_event->execute()) {
    $retorna = ['sit' => true, 'msg' => '<div class="alert alert-success" role="alert">Evento cadastrado com sucesso!</div>'];
    $_SESSION['msg'] = '<div class="alert alert-success" role="alert">Evento cadastrado com sucesso!</div>';
} else {
    $retorna = ['sit' => false, 'msg' => '<div class="alert alert-danger" role="alert">Erro: Evento não foi cadastrado com sucesso!</div>'];
}

header('Content-Type: application/json');
echo json_encode($retorna);
